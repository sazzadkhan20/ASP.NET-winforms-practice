...
{
    public partial class Forgot_Password_OTP : Form
    {
        private int otp;
        private string to;
        private string database_name;
        public Forgot_Password_OTP(string to, int otp, string database_name)
        {
            InitializeComponent();
            this.otp = otp;
            this.to = to;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (otpEntered.Length == 6 && otpEntered.All(char.IsDigit))
            {
                ...
                if (int.TryParse(otpEntered, out int enteredOtp) && enteredOtp == otp)
                {
                    ...
                    Change_Password Change_Password = new Change_Password(to,database_name);
                    Change_Password.Show();
                    this.Hide();
                    Change_Password.FormClosed += (s, args) => this.Close();
                }
                else
                {
